...
for syst in systematics:
	...
	for bkg in procList:
		for cat in catList:
			...
			hsUp = RFile.Get(Prefix+'__lowess'+syst+upTag).Clone(Prefix+'_sup')
			hsDn = RFile.Get(Prefix+'__lowess'+syst+downTag).Clone(Prefix+'_sdn')
			hUp.Divide(hNm)
			hDn.Divide(hNm)
			hsUp.Divide(hNm)
			hsDn.Divide(hNm)
			...
			canv.SaveAs(tempVersion+'/'+saveDir+'/'+syst+'/'+syst+'_'+cat+'_'+bkg+'.png')
RFile.Close()
