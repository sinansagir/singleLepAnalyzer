...
def get_model():
    model = build_model_from_rootfile(input,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0))
...
matrices = ROOT.TFile('mle_covcorr.root','RECREATE')
cov_hist.Write()
corr_hist.Write()
matrices.Close()
