...
input0H = '/user_data/jhogan/CMSSW_7_4_14/src/tptp_2016/makeTemplates/control_NewEl/splitLess/templates_minMlbST_TTM1000_bW0p5_tZ0p25_tH0p25_36p814fb_rebinned_stat0p3.root'
...
def get0H_model():
    if category == 'All': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0))
    if category == 'Higgs': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH0')==0))
    if category == 'Higgs1b': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0  and s.count('nH0')==0 and s.count('nH2b')==0))
    if category == 'Higgs2b': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH0')==0 and s.count('nH1b')==0))
    if category == 'Ttbar': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0_')==0 and s.count('nW1p_')==0))
    if category == 'Ttbar1b': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0_')==0 and s.count('nW1p_')==0 and s.count('nB2')==0 and s.count('nB3p')==0))
    if category == 'Ttbar2b': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0_')==0 and s.count('nW1p_')==0 and s.count('nB1')==0 and s.count('nB3p')==0))
    if category == 'Ttbar3b': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0_')==0 and s.count('nW1p_')==0 and s.count('nB1')==0 and s.count('nB2')==0))
    if category == 'Wjets': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0p_')==0))
    if category == 'Wjets0W': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0p_')==0 and s.count('nW1p')==0))
    if category == 'Wjets1W': model = build_model_from_rootfile(input0H,include_mc_uncertainties=True,histogram_filter = (lambda s: s.count('sig')==0 and s.count('nH1b')==0 and s.count('nH2b')==0 and s.count('nW0p_')==0 and s.count('nW0_')==0))
    ...
    try: model.add_lognormal_uncertainty('luminosity', math.log(1.026), '*', '*')
    except RuntimeError: pass
