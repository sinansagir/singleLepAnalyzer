...
def get_model():
    ...
    for obs in obsvs:
		...
		if 'nT0p_nW0_nB1' in obs:
			...
			if 'ewk' in obs: model.add_lognormal_uncertainty('ewk0W1BSys',  math.log(1.10), 'ewk', obs) #0 W-tag
		if 'nT0p_nW0_nB2p' in obs:
			if 'top' in obs: model.add_lognormal_uncertainty('top0W2pBSys', math.log(1.15), 'top', obs) #2+ b-tag
			if 'ewk' in obs: model.add_lognormal_uncertainty('ewk0W2pBSys', math.log(1.10), 'ewk', obs) #0 W-tag
		if 'nT0p_nW1p_nB1' in obs:
			if 'top' in obs: model.add_lognormal_uncertainty('top1pW1BSys', math.log(1.11), 'top', obs) #1 b-tag
			if 'ewk' in obs: model.add_lognormal_uncertainty('ewk1pW1BSys', math.log(1.13), 'ewk', obs) #1+ W-tag
		if 'nT0p_nW1p_nB2p' in obs:
			if 'top' in obs: model.add_lognormal_uncertainty('top1pW2pBSys',math.log(1.15), 'top', obs) #2+ b-tag
			if 'ewk' in obs: model.add_lognormal_uncertainty('ewk1pW2pBSys',math.log(1.13), 'ewk', obs) #1+ W-tag
