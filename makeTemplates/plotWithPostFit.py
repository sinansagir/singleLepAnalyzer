...
limitDir=os.getcwd()+'/../thetaLimits/templates_M17WtSF_2017_3_31_SRpCRplots/postfit/test/'

isRebinned='_rebinned_stat0p3' #post for ROOT file names
saveKey = '_postfit3' # tag for plot names

sig1='X53X53M1000left' #  choose the 1st signal to plot
sig1leg='X_{5/3}#bar{X}_{5/3} LH (1.0 TeV)'
sig2='X53X53M1300right' #  choose the 2nd signal to plot
sig2leg='X_{5/3}#bar{X}_{5/3} RH (1.3 TeV)'
scaleSignals = False
...
RFilePost = rt.TFile(limitDir+'/histos-mle_All.root')
...
for tag in tagList:
	...
	for isEM in isEMlist:
			...
			for syst in systematicList+q2list:
				...
				for ud in ['minus','plus']:
					for proc in bkgProcList:
						...
						RFileSyst = rt.TFile(limitDir+RFileName)
						try: 
		...
		if drawQCD: leg = rt.TLegend(0.42,0.52,0.93,0.87)
		if not drawQCD or blind: leg = rt.TLegend(0.42,0.62,0.93,0.87)
	...
	if doAllSys:
		...
		for proc in bkgProcList:
			...
			totUpShiftPrctg=0
			totDnShiftPrctg=0
			nomHist = bkghistsmerged[proc+'isL'+tagStr].Integral()
			for syst in systematicList+q2list:
				for ud in ['minus','plus']:
					try:
			...
			yieldsErrsAll[proc+'isL'+tagStr] += (shpSystUncPrctg*nomHist)**2
...
# 		totUpShiftPrctg=0 #this block is for adding signal systematics to postfit yield table, but it doesn't yet work -- needs for reading of the signal systematics from postfit templates above!
# 		totDnShiftPrctg=0
# 		nomHist = hsig1merged.Integral()
# 		for syst in systematicList+q2list:
# 			for ud in ['minus','plus']:
# 				try:
# 					shpHist = systHists[sig1+'isL'+tagStr+syst+ud].Integral()
# 					shift = shpHist/(nomHist+1e-20)-1
# 					if shift>0.: totUpShiftPrctg+=shift**2
# 					if shift<0.: totDnShiftPrctg+=shift**2
# 				except Exception as e: 
...
# 					pass
# 		shpSystUncPrctg = (math.sqrt(totUpShiftPrctg)+math.sqrt(totDnShiftPrctg))/2 #symmetrize the total shape uncertainty up/down shifts
# 		yieldsErrsAll[sig1+'isL'+tagStr] += (shpSystUncPrctg*nomHist)**2
# 		totUpShiftPrctg=0
# 		totDnShiftPrctg=0
# 		nomHist = hsig2merged.Integral()
# 		for syst in systematicList+q2list:
# 			for ud in ['minus','plus']:
# 				try:
# 					shpHist = systHists[sig2+'isL'+tagStr+syst+ud].Integral()
# 					shift = shpHist/(nomHist+1e-20)-1
# 					if shift>0.: totUpShiftPrctg+=shift**2
# 					if shift<0.: totDnShiftPrctg+=shift**2
# 				except Exception as e: 
...
# 					pass
# 		shpSystUncPrctg = (math.sqrt(totUpShiftPrctg)+math.sqrt(totDnShiftPrctg))/2 #symmetrize the total shape uncertainty up/down shifts
# 		yieldsErrsAll[sig2+'isL'+tagStr] += (shpSystUncPrctg*nomHist)**2
	...
	if drawQCDmerged: legmerged = rt.TLegend(0.42,0.52,0.93,0.87)
	if not drawQCDmerged or blind: legmerged = rt.TLegend(0.42,0.62,0.93,0.87)
