...
blind = False
yLog  = True
if len(sys.argv)>4: yLog=bool(sys.argv[4])
...
def formatUpperHist(histogram,th1hist):
	...
	if blind == True:
		...
		if 'deltaRAK8' in histogram.GetName(): histogram.GetYaxis().SetTitleOffset(0.93)

	if 'Pruned' in histogram.GetName(): histogram.GetYaxis().SetTitle("Jets / 10 GeV")
	if 'NsubB' in histogram.GetName(): histogram.GetYaxis().SetTitle("Jets")
	if 'Tau' in histogram.GetName(): histogram.GetYaxis().SetTitle("Jets / 0.02")
	if 'deltaR' in histogram.GetName(): histogram.GetYaxis().SetTitle("Events / 0.1")
	if 'PrunedWNm1' in histogram.GetName(): histogram.GetXaxis().SetTitle("AK8 jet pruned mass [GeV]")
	if 'PrunedHNm1' in histogram.GetName(): histogram.GetXaxis().SetTitle("AK8 jet pruned mass [GeV]")
	if 'Tau21Nm1' in histogram.GetName(): histogram.GetXaxis().SetTitle("AK8 jet #tau_{2}/#tau_{1}")
	if 'PrunedNsubBNm1' in histogram.GetName(): histogram.GetXaxis().SetTitle("b-tagged subjet multiplicity");
	if 'deltaRAK8' in histogram.GetName(): histogram.GetXaxis().SetTitle("#DeltaR(leading AK8, other AK8)")
	if 'minMlb' in histogram.GetName(): 
		if 'nB0' in histogram.GetName(): histogram.GetXaxis().SetTitle("min[M(l,j)] [GeV]")
		else: histogram.GetXaxis().SetTitle("min[M(l,b)] [GeV]")

	# #histogram.GetYaxis().CenterTitle()
	histogram.SetMinimum(2.01e-4)
	if 'minMlb' in histogram.GetName():
		if 'B3p' in histogram.GetName() or 'H1b' in histogram.GetName(): histogram.SetMinimum(1.01e-4)
		if 'H2b' in histogram.GetName(): histogram.SetMinimum(1.01e-5)
	if 'minMlb' not in histogram.GetName(): histogram.SetMinimum(0.000101)
	if not yLog: 
		histogram.SetMinimum(0.25)
		if 'minMlb' not in histogram.GetName(): histogram.SetMinimum(0.05)
		if 'deltaR' in histogram.GetName() or 'Tau' in histogram.GetName(): histogram.GetYaxis().SetRangeUser(0.25,2500) #histogram.SetMaximum(1.15*histogram.GetMaximum())
		if 'PrunedWNm1' in histogram.GetName(): histogram.SetMaximum(1.15*histogram.GetMaximum())
	if yLog:
		if 'NsubB' in histogram.GetName(): histogram.SetMinimum(0.25)
		uPad.SetLogy()
		if not doNormByBinWidth: 
			if 'NsubB' in histogram.GetName() or 'NWJets' in histogram.GetName(): histogram.SetMaximum(1000*histogram.GetMaximum())
			else: histogram.SetMaximum(50*histogram.GetMaximum())
		else: 
			if 'nB1_' in histogram.GetName(): histogram.SetMaximum(1000*histogram.GetMaximum())
			else: histogram.SetMaximum(50*histogram.GetMaximum())
...
for tag in tagList:
	...
	for isEM in isEMlist:
		...
		bkgHT = bkghists[bkgProcList[2]+catStr].Clone()
		for proc in bkgProcList:
			if proc==bkgProcList[2]: continue
		...
		yvals = gaeBkgHT.GetY()
		print 'bkgHT = ',bkgHT.GetBinContent(1),'+/-',bkgHT.GetBinError(1)
		print 'gaeBkgHT = ',yvals[0],'+',gaeBkgHT.GetErrorYhigh(0),'-',gaeBkgHT.GetErrorYlow(0)
		...
		for ibin in range(1,bkghists[bkgProcList[2]+catStr].GetNbinsX()+1):
			errorUp = 0.
			errorDn = 0.
			...
			#if ibin < 3:
			#	print 'bkgHT = ',bkgHT.GetBinContent(ibin),'+/-',bkgHT.GetBinError(ibin)
			#	print 'gaeBkgHT = ',yvals[ibin-1],'+',math.sqrt(errorStatUp),'-',math.sqrt(errorStatDn)
		...
		if yLog and not blind: uPad=TPad("uPad","",0,yDiv,1,1) #for actual plots
		...
		chLatex.SetTextAlign(31) # align center
		flvString = ''
		tagString = ', '
		...
		if iPlot != 'YLD':
			if iPlot != 'deltaRAK8': 
				if drawQCD: 
					chLatex.DrawLatex(0.92, 0.48, flvString+tagString)
					#chLatex.DrawLatex(0.92, 0.42, tagString)
				else: 
					chLatex.DrawLatex(0.92, 0.56, flvString+tagString)
					#chLatex.DrawLatex(0.92, 0.50, tagString)
			else: 
				chLatex.DrawLatex(0.85,0.81,flvString)
				chLatex.DrawLatex(0.85,0.75,tagString)

		if drawQCD: 
			leg = TLegend(0.350,0.542,0.939,0.89)
			if iPlot == 'deltaRAK8': leg = TLegend(0.15,0.52,0.55,0.82)
		if not drawQCD or blind: 
			leg = TLegend(0.350,0.622,0.939,0.89)
		...
		prelimTex3.DrawLatex(0.16,0.75,"Supplementary")
		...
		#c1.Write()
		savePrefix = templateDir.replace(cutString,'')+templateDir.split('/')[-2]+'plotsPaper/'
		...
		if doOneBand:
			...
			c1.SaveAs(savePrefix+"_suppl.pdf")
			c1.SaveAs(savePrefix+"_suppl.png")
			c1.SaveAs(savePrefix+"_suppl.eps")
			c1.SaveAs(savePrefix+"_suppl.root")
			c1.SaveAs(savePrefix+"_suppl.C")
		for proc in bkgProcList:
	...
	bkgHTmerged = bkghistsmerged[bkgProcList[2]+'isL'+tagStr].Clone()
	for proc in bkgProcList:
		if proc==bkgProcList[2]: continue
	...
	for ibin in range(1,bkghistsmerged[bkgProcList[2]+'isL'+tagStr].GetNbinsX()+1):
		errorUp = 0.
		errorDn = 0.
	...
	formatUpperHist(gaeDatamerged,hDatamerged)
	uPad.cd()
	...
	chLatexmerged.SetTextAlign(31) # align center
	flvString = 'e/#mu+jets'
	tagString = ', '
	if tag[0]!='0p':
		if '1b' in tag[0]: tagString+='1b H, '
	...
	if iPlot != 'YLD':
		if iPlot != 'deltaRAK8': 
			if drawQCDmerged: 
				chLatexmerged.DrawLatex(0.92, 0.48, flvString+tagString)
				#chLatexmerged.DrawLatex(0.92, 0.42, tagString)
			else: 
				chLatexmerged.DrawLatex(0.92, 0.56, flvString+tagString)
				#chLatexmerged.DrawLatex(0.92, 0.50, tagString)
		else: 
			chLatexmerged.DrawLatex(0.85,0.81,flvString)
			chLatexmerged.DrawLatex(0.85,0.75,tagString)

	if drawQCDmerged: 
		legmerged = TLegend(0.350,0.542,0.939,0.89)
		if iPlot == 'HT': legmerged = TLegend(0.374,0.544,0.951,0.890)
		if iPlot == 'deltaRAK8': legmerged = TLegend(0.15,0.24,0.55,0.74)
	if not drawQCDmerged or blind: 
		legmerged = TLegend(0.350,0.622,0.939,0.89)
	...
	prelimTex3.SetTextSize(0.055)
	prelimTex3.SetLineWidth(2)
	if iPlot != 'deltaRAK8': prelimTex3.DrawLatex(0.16,0.75,"Supplementary")
	else: prelimTex3.DrawLatex(0.29,0.81,"Supplementary")
	...
	#c1merged.Write()
	savePrefixmerged = templateDir.replace(cutString,'')+templateDir.split('/')[-2]+'plotsPaper/'
	...
	if doOneBand: 
		...
		c1merged.SaveAs(savePrefixmerged+"_suppl.pdf")
		c1merged.SaveAs(savePrefixmerged+"_suppl.png")
		c1merged.SaveAs(savePrefixmerged+"_suppl.eps")
		c1merged.SaveAs(savePrefixmerged+"_suppl.root")
		c1merged.SaveAs(savePrefixmerged+"_suppl.C")
	for proc in bkgProcList:
