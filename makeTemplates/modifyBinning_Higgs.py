...
cutString = 'split/'#BB_templates/'
templateDir = os.getcwd()+'/templates_minMlb_PaperARC_fixesBB/'+cutString #BoostedHiggs/'+cutString
...
rfiles = [file for file in findfiles(templateDir, '*.root') if 'rebinnedDV' in file and skipcode in file]
...
for rfile in rfiles: 
	...
	for hist in allhists:
            ...
            #if 'ScaleVar' not in hist and 'QCD__jsf' not in hist and 'DYJets__jsf' not in hist and 'SingleTop__jsf' not in hist and 'Diboson__jsf' not in hist and signame+'__jsf' not in hist: rebinnedHists[hist].Write()		    
        ...
        # muRUphists = [k.GetName() for k in tfiles[iRfile].GetListOfKeys() if 'ScaleVar'+upTag in k.GetName()]
        # newMuRFNameBase = 'ScaleVar'
        ...
        #     muRFcorrdNewUpHist = rebinnedHists[hist].Clone(hist.replace('ScaleVar'+upTag,newMuRFName+upTag))
        #     muRFcorrdNewDnHist = rebinnedHists[hist.replace(upTag,downTag)].Clone(hist.replace('ScaleVar'+upTag,newMuRFName+downTag))
