...
removalKeys['jmr__'] = False
removalKeys['jms__'] = False
...
print templateDir
rfiles = [file for file in findfiles(templateDir, '*.root') if 'rebinned' not in file and brcode+'0p5' in file and 'M1000' in file and combinefile not in file and '_'+iPlot+'_' in file.split('/')[-1]]
	...
	if ('H1b' not in chn and 'H2b' not in chn and 'H1p' not in chn) or iPlot != 'minMlbST':
		...
		for iBin in range(1,Nbins+1):
			...
			if totTempBinContent_E>0. and totTempBinContent_M>0.:
				if 'ttbar' in templateDir or 'wjets' in templateDir or 'higgs' in templateDir or (totTempSigContent_E>0. and totTempSigContent_M>0):
		...
		for iBin in range(1,Nbins+1):
			...
			totTempSigContent_M += SigHists[chn.replace('isE','isM')].GetBinContent(Nbins+1-iBin)
			if totTempBinContent_E>0. and totTempBinContent_M>0.:
				if 'ttbar' in templateDir or 'wjets' in templateDir or 'higgs' in templateDir or SigHists[chn].GetXaxis().GetBinLowEdge(Nbins+1-iBin) < 1351.0 or (totTempSigContent_E>0. and totTempSigContent_M>0.):
					if math.sqrt(totTempBinErrSquared_E)/totTempBinContent_E<=stat and math.sqrt(totTempBinErrSquared_M)/totTempBinContent_M<=stat:
						totTempBinContent_E = 0.
						totTempBinContent_M = 0.
						totTempBinErrSquared_E = 0.
...
for rfile in rfiles: 	
	...
	outputRfiles[iRfile] = TFile(rfile.replace('.root','_rebinned_splitJMSR_stat'+str(stat).replace('.','p')+'.root'),'RECREATE')	
	...
	for chn in channels:
		...
		#Constructing muRF shapes
		jmrhists = [k.GetName() for k in tfiles[iRfile].GetListOfKeys() if 'jmr'+upTag in k.GetName() and chn in k.GetName()]
		jmrNameBase = 'jmr'
		for hist in jmrhists:
			if 'nH0_' in hist: jmrName = jmrNameBase+'W'
			elif 'nH1' in hist or 'nH2' in hist: jmrName = jmrNameBase+'H'
			else: print 'jmr doesnt understand this name:'+hist
			jmrUpHist = rebinnedHists[hist].Clone(hist.replace('jmr'+upTag,jmrName+upTag))
			jmrDnHist = rebinnedHists[hist.replace(upTag,downTag)].Clone(hist.replace('jmr'+upTag,jmrName+downTag))
			jmrUpHist.SetDirectory(0)
			jmrDnHist.SetDirectory(0)
			jmrUpHist.Write()
			jmrDnHist.Write()
		...
		#Constructing muRF shapes
		jmshists = [k.GetName() for k in tfiles[iRfile].GetListOfKeys() if 'jms'+upTag in k.GetName() and chn in k.GetName()]
		jmsNameBase = 'jms'
		for hist in jmshists:
			if 'nH0_' in hist: jmsName = jmsNameBase+'W'
			elif 'nH1' in hist or 'nH2' in hist: jmsName = jmsNameBase+'H'
			else: print 'jms doesnt understand this name:'+hist
			jmsUpHist = rebinnedHists[hist].Clone(hist.replace('jms'+upTag,jmsName+upTag))
			jmsDnHist = rebinnedHists[hist.replace(upTag,downTag)].Clone(hist.replace('jms'+upTag,jmsName+downTag))
			jmsUpHist.SetDirectory(0)
			jmsDnHist.SetDirectory(0)
			jmsUpHist.Write()
			jmsDnHist.Write()
