...
if iPlot=='BDTYLD': 
	ttProcList = ['ttnobb','ttbb']#['ttbb','ttnobb']
	useCombineTemplates = False
	scaleSignals = False

# ROOT.TColor.GetColor(hex_code_string)

ttH_color=rt.TColor.GetColor('#163d4e')
top_color=rt.TColor.GetColor('#54792f')
ewk_color=rt.TColor.GetColor('#d07e93')
ttnobb_color=rt.TColor.GetColor('#cf9ddb')
qcd_color=rt.TColor.GetColor('#c1caf3')
ttbb_color=rt.TColor.GetColor('#d3eeef')
# rt.TColor.GetColor('')
...
systematicList += ['hotstat','hotcspur','hotclosure']
...
def formatUpperHist(histogram,histogramBkg):
	...
	if yLog:
		uPad.SetLogy()
		if 'YLD' in iPlot: 
			histogram.SetMaximum(2e4*histogramBkg.GetMaximum())
			histogram.SetMinimum(0.101)
		elif not isCategorized:
			histogram.SetMaximum(5e6*histogramBkg.GetMaximum())
		else: histogram.SetMaximum(2e8*histogramBkg.GetMaximum())
	else: 
...
for catEStr in catsElist:
	...
	# if 'nB0p' not in catEStr or iPlot=='HTYLD': systematicList_ += []#,'btag']
