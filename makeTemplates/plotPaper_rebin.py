...
def formatUpperHist(histogram,th1hist):
	...
	if 'minMlb' not in histogram.GetName(): histogram.SetMinimum(0.00101)
	if not yLog: 
		histogram.SetMinimum(0.25)
		if 'minMlb' not in histogram.GetName(): histogram.SetMinimum(0.05)
		if 'deltaR' in histogram.GetName() or 'Tau' in histogram.GetName(): histogram.SetMaximum(1.15*histogram.GetMaximum())
...
for tag in tagList:
	...
	for isEM in isEMlist:
				...
				for ud in ['minus','plus']:
					for proc in bkgProcList:
						try: 
							...
							systHists[proc+catStr+syst+ud].Rebin(2)
		...
		if iPlot != 'YLD':
			if iPlot != 'deltaRAK8': 
				...
				chLatex.DrawLatex(0.75,0.84,flvString)
				chLatex.DrawLatex(0.75,0.78,tagString)
		...
		if doOneBand:
			...
			c1.SaveAs(savePrefix+"_rebinned2.pdf")
			c1.SaveAs(savePrefix+"_rebinned2.png")
			c1.SaveAs(savePrefix+"_rebinned2.eps")
			c1.SaveAs(savePrefix+"_rebinned2.root")
			c1.SaveAs(savePrefix+"_rebinned2.C")
		for proc in bkgProcList:
	...
	for proc in bkgProcList:
		try: 
			...
			bkghistsmerged[proc+'isL'+tagStr].Rebin(2)
	...
	if iPlot=='PrunedHNm1': gaeDatamerged.SetMaximum(1.5*max(gaeDatamerged.GetMaximum(),bkgHTmerged.GetMaximum()))
	...
	if iPlot != 'YLD':
		if iPlot != 'deltaRAK8': 
			...
			chLatexmerged.DrawLatex(0.75,0.84,flvString)
			chLatexmerged.DrawLatex(0.75,0.78,tagString)

	if drawQCDmerged: 
		legmerged = TLegend(0.350,0.542,0.939,0.89)
		if iPlot == 'deltaRAK8': legmerged = TLegend(0.15,0.32,0.55,0.82)
	...
	if doOneBand: 
		...
		c1merged.SaveAs(savePrefixmerged+"_rebinned2.pdf")
		c1merged.SaveAs(savePrefixmerged+"_rebinned2.png")
		c1merged.SaveAs(savePrefixmerged+"_rebinned2.eps")
		c1merged.SaveAs(savePrefixmerged+"_rebinned2.root")
		c1merged.SaveAs(savePrefixmerged+"_rebinned2.C")
	for proc in bkgProcList:
