...
from ROOT import gROOT,TGraph,TCanvas,TLatex,TLine,TLegend,TGraphAsymmErrors
...
lumiPlot = '35.9'
lumiStr = '36p814'
distribution = 'minMlbST'
signal = 'TT'
chiral = ''
limitDir='/user_data/jhogan/CMSSW_7_4_14/src/tptp_2016/optimization/limits/templates_minMlbST_2017_3_3/minMlbST'
postfix = '_2015cuts_' # for plot names in order to save them as different files
...
massPoints = [900,1000,1100,1200,1300,1400]#,1500,1600]
...
cutStrings = ['lep60_MET75_NJets3_NBJets0_DR3.0_1jet300_2jet150_3jet100',
 			  'lep60_MET60_NJets3_NBJets0_DR3.0_1jet300_2jet200_3jet80',
 			  'lep60_MET60_NJets3_NBJets0_DR3.0_1jet400_2jet200_3jet80',
...
bestSelection = 'lep60_MET60_NJets3_NBJets0_DR3.0_1jet300_2jet200_3jet80'
...
for cutString in cutStrings:
	...
	significance[cutString] = TGraphAsymmErrors(len(mass))
	...
	for i in range(len(mass)):
		try:
			...
			fsig.close()
			expSig = linesSig[0][0]
			expSigDn = linesSig[1][0]
			expSigUp = linesSig[2][0]
			if mass[i]==comMass:
			...
			#significance[cutString].SetPointEYhigh(i,expSigUp)
			#significance[cutString].SetPointEYlow(i,expSigDn)
		except: 
	...
	significance[cutString].SetFillColor(ind)
	significance[cutString].SetFillStyle(3004)
#os._exit(1)
...
legend = TLegend(0.5,0.7,0.93,0.93)

for key in significance.keys():
	if 'MET75' in key: legend.AddEntry(significance[key],"2015 cuts 75/300/150/100","l")
	elif '300' in key: legend.AddEntry(significance[key],"1 TeV 60/300/200/80","l")
	elif '400' in key: legend.AddEntry(significance[key],"1.1 TeV 60/400/200/80","l")
...
# 	if 'NBJets' in sel or 'lep' in sel or 'NJets' in sel or 'DR' in sel: continue
# 	variedCut = ''
