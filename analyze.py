...
def analyze(tTree,process,cutList,isotrig,doAllSys,doJetRwt,iPlot,plotDetails,category,region,isCategorized):
	...
	if 'CR' in region:
		...
		#if region=='TTCR': cut += ' && NJetsCSV_JetSubCalc >= 2'
		#elif region=='WJCR': cut += ' && NJetsCSV_JetSubCalc == 0'
	else: # 'PS' or 'SR'
	...
	jetSFstr='1'
	if doJetRwt and (process!='WJetsMG' and ('WJetsMG' in process or 'QCD' in process)): jetSFstr= 'JetSF_80X'
	...
	nHtagLJMETname = 'NJetsH1btagged' #shifts will be: nHtagLJMETname+'up/dn'
	nWtagLJMETname = 'NJetsWtagged_0p6' #shifts will be: nWtagLJMETname+'_shifts[0/1/2/3/4/5]'
	nbtagLJMETname = 'NJetsCSV_JetSubCalc' #shifts will be: nbtagLJMETname+'_shifts[0/1/2/3]'
	...
	# replace cuts for shifts
        # cut_btagUp = fullcut.replace('NJetsHtagged','NJetsHtagged_shifts[0]').replace(nbtagLJMETname,nbtagLJMETname+'_shifts[0]')
        # cut_btagDn = fullcut.replace('NJetsHtagged','NJetsHtagged_shifts[1]').replace(nbtagLJMETname,nbtagLJMETname+'_shifts[1]')
        # cut_mistagUp = fullcut.replace('NJetsHtagged','NJetsHtagged_shifts[2]').replace(nbtagLJMETname,nbtagLJMETname+'_shifts[2]')
        # cut_mistagDn = fullcut.replace('NJetsHtagged','NJetsHtagged_shifts[3]').replace(nbtagLJMETname,nbtagLJMETname+'_shifts[3]')
